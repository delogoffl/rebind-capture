...
export const DEFAULTS = Object.freeze({
   ...
   *
   * These are the defaults the editor opens with, not a property of any mark
   * already made — a stored annotation carries its own colour and weight, so
   * changing this never alters an image that already exists.
   */
  annotateColor: 'red',
  annotateWeight: 'md',
  /**
   * Offer a box around whatever changed since the previous step.
   *
   * A suggestion only: it draws dashed candidates and waits to be clicked.
   * Placing the box is the part of writing documentation people skip, but an
   * automatic box that silently lands on the wrong thing is worse than none in
   * a document somebody is about to attach to a ticket.
   */
  autoHighlight: true,

  /**
   * Record a SHA-256 for every file as it is written.
   *
   * Costs a hash of a few megabytes per capture, which is not measurable next
   * to encoding the PNG, and is what later lets an export be checked for
   * alteration. Off means captures are stored with no digest and exports say
   * so rather than quietly claiming less than they should.
   */
  hashAssets: true,
  /** Ship `manifest.json` and `SHA256SUMS` inside zip exports. */
  exportManifest: true,

  /**
   * Turn a recording into numbered steps.
   *
   * The input hook already runs during a take for the keypress HUD; this keeps
   * what it saw, so the frame at each click can be pulled out afterwards. The
   * extraction itself is always a button — never automatic — because sixty
   * steps appearing in the library unasked is not a feature.
   */
  autoSteps: true,
  /** 'clicks' — clicks only. 'actions' — clicks, typing runs and named keys. */
  autoStepsOn: 'actions',
  /** Ceiling on how many steps one recording may produce. */
  autoStepsMax: 40,
...
const ENUMS = {
  ...
  annotateColor: ['red', 'amber', 'green', 'cyan', 'indigo'],
  annotateWeight: ['sm', 'md', 'lg'],
  autoStepsOn: ['clicks', 'actions']
}
