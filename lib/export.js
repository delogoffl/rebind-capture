...
import {
  buildManifest, manifestDigest, manifestText, checksumFile,
  MANIFEST_FILE, CHECKSUM_FILE
} from './manifest.js'
...
export async function buildExport(format, options) {
  const {
    ...
    machine = {},
    /**
     * Ship the integrity manifest alongside the files.
     *
     * Only the zip formats can carry it — a single PNG or a bare .mp4 has
     * nowhere to put a second file, and writing a loose `SHA256SUMS` next to
     * whatever the user picked would litter their Downloads folder. Those
     * formats are honest about it rather than pretending: no manifest, no
     * claim.
     */
    manifest: wantManifest = true,
    onProgress
  } = options
  ...
  if (format === 'png') {
    ...
    if (wantManifest) {
      const { files } = await integrity({
        session, steps, machine, prefix: base,
        stepPath: (step) => `step-${pad(step.index)}.png`
      })
      entries.push(...files)
    }
  ...
  if (format === 'md') {
    ...
    // The manifest covers the assets; the report quotes the manifest's digest.
    // One direction only, so neither has to be hashed after the other exists.
    let digest = null
    if (wantManifest) {
      // Steps only. The report *lists* the recordings but the zip does not
      // contain them, and a manifest that claims a file the pack does not hold
      // makes an intact pack verify as one with missing files.
      const built = await integrity({
        session, steps, machine, prefix: base,
        stepPath: (step) => `steps/step-${pad(step.index)}.png`
      })
      digest = built.digest
    ...
    entries.unshift({
      path: `${base}/report.md`,
      data: markdown(session, steps, { includeMeta, machine, media, manifest: includeMeta ? digest : null })
    })
...
/**
 * The two files that make a pack checkable.
 *
 * Returned as zip entries with the manifest's own digest attached, so the
 * caller can quote it in a report without rebuilding the manifest to find it.
 * The digests themselves were computed when the assets were written — this only
 * collects them, which is deliberate: hashing at export time would certify the
 * files as they are at the moment an alteration would already have happened.
 */
async function integrity({ session, steps, media = [], machine, prefix, stepPath }) {
  const manifest = buildManifest({
    session,
    steps,
    media,
    machine,
    paths: { step: stepPath, media: (item) => item.file }
  })
  const digest = await manifestDigest(manifest)
  manifest.self = digest

  const files = [
    { path: `${prefix}/${MANIFEST_FILE}`, data: manifestText(manifest) }
  ]
  const sums = checksumFile(manifest)
  // No digests means no checksum file, rather than an empty one that reports
  // success against nothing.
  if (sums) files.push({ path: `${prefix}/${CHECKSUM_FILE}`, data: sums })
  return { files, digest }
}
